...
import { HubConnectionBuilder, HttpTransportType, JsonHubProtocol, LogLevel, HubConnection } from '@microsoft/signalr'
...
export const SignalRHub = {
  ...
  addHubListener: async (_callback, _response) => {
    try {
      console.log('addHubListener _hubConnection 1', await _hubConnection)
      console.log('addHubListener _hubConnection methods', await _hubConnection._methods)
      console.log('addHubListener _hubConnection 2', await _hubConnection)
      // console.log('response', eval(_response))
      // const evres = eval(_response)
      const hubStatus = await _hubConnection.state
      if(hubStatus === 'Connected') {
        await _hubConnection.on(_callback, async (_response) => {
