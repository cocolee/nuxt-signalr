...
export class signalR {
  ...
  async ConnectHub() {
    ...
    connection.onclose(async (e) => {
      ...
      if (this._hubConnection.connectionStarted === false) {
        if (this._hubConnection.connectionState !== "Connected") {
          // await this.Connect()
          // eslint-disable-next-line no-console
          console.log('start ediyorum abiii.')
          await this.StartHub();
          // eslint-disable-next-line no-console
          console.log('abi ReconnectHubGroup. cagirdim abi.')
    ...
    connection.onreconnecting(async (e) => {
      // eslint-disable-next-line no-console
      console.log('connection onclose : ', await this._hubConnection)
      // eslint-disable-next-line no-console
      console.log("connection.HubName : ", await this._hubConnection);
      // eslint-disable-next-line no-console
      console.log(
  ...
  async JoinHubGroupWPro(_callback) {
    await new Promise((resolve, reject) => {
      if (Array.isArray(_callback)) {
        ...
        if (value !== undefined && value2 === undefined) {
          this._hubConnection.invoke(_name, value).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
        } else if (value !== undefined && value2 !== undefined) {
          this._hubConnection.invoke(_name, value, value2).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
        } else if (value === undefined && value2 === undefined){
          this._hubConnection.invoke(_name).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
  ...
  async LeaveHubGroupWPro(_callback) {
    // eslint-disable-next-line no-console
    console.log('_hubName _callback ======>' + ' ' + _callback)
    await new Promise((resolve, reject) => {
      if (Array.isArray(_callback)) {
        ...
        if (value !== undefined && value2 === undefined) {
          this._hubConnection.invoke(name, value).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
        } else if (value !== undefined && value2 !== undefined) {
          this._hubConnection.invoke(name, value, value2).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
        } else if (value === undefined && value2 === undefined) {
          this._hubConnection.invoke(name).then((res) => {
            resolve(res)
          }).catch((ex) => {
            reject(ex)
          });
